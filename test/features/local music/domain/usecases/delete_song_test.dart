...
void main() {
  ...
  test(
    'should delete song from playlists, analytics, and finally from music repository',
    () async {
      // ARRANGE
      when(
        () => mockPlaylistRepository.removeSongFromAllPlaylists(any()),
      ).thenAnswer((_) async => const Right(null));
      when(
        () => mockAnalyticsRepository.deleteSongAnalytics(any()),
      ).thenAnswer((_) async => const Right(null));
      when(
        () => mockMusicRepository.deleteSong(any(), any()),
      ...
      verify(
        () => mockPlaylistRepository.removeSongFromAllPlaylists(tSong.id),
      ).called(1);
      verify(
        () => mockAnalyticsRepository.deleteSongAnalytics(tSong.id),
      ).called(1);
      verify(
        () => mockMusicRepository.deleteSong(tSong.id, tSong.path),
      ).called(1);
    },
  );
