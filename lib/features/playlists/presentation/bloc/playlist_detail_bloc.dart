...
class PlaylistDetailBloc extends Bloc<PlaylistDetailEvent, PlaylistDetailState> {
  ...
  Future<void> _onLoadPlaylistDetail(_LoadPlaylistDetail event, Emitter<PlaylistDetailState> emit) async {
    ...
    result.fold(
      ...
      (songs) => emit(PlaylistDetailState.loaded(playlist: event.playlist, songs: songs)),
    );
  }

  Future<void> _onAddSong(_AddSong event, Emitter<PlaylistDetailState> emit) async {
    final currentState = state;
    if (currentState is _Loaded) {
      final result = await _addSongToPlaylist(AddSongToPlaylistParams(
  ...
  Future<void> _onRemoveSong(_RemoveSong event, Emitter<PlaylistDetailState> emit) async {
    final currentState = state;
    if (currentState is _Loaded) {
      final result = await _removeSongFromPlaylist(RemoveSongFromPlaylistParams(
        playlistId: currentState.playlist.id,
        songId: event.songId,
      ));
  ...
  Future<void> _onEditPlaylistDetail(_EditPlaylistDetail event, Emitter<PlaylistDetailState> emit) async {
    final currentState = state;
    if (currentState is _Loaded) {
      final result = await _editPlaylist(EditPlaylistParams(
